...
class GlobeNotificationController {
  ...
  Future<void> setNotificationAlarm() async {
    ...
    if (notificationTime != null && notificationTime.length == 2) {
      targetDateTime = new DateTime(now.year, now.month, now.day,
          int.parse(notificationTime[0]), int.parse(notificationTime[1]), 00);
      if (targetDateTime.isBefore(now)) {
        targetDateTime = new DateTime(
                now.year,
                now.month,
                now.day,
                int.parse(notificationTime[0]),
                int.parse(notificationTime[1]),
                00)
            .add(const Duration(days: 1));
      }
      await AndroidAlarmManager.oneShotAt(
          targetDateTime, alarmId,
          callback,
          exact: true,
          alarmClock: true,
  ...
  static Future<void> setNotification() async {
    ...
    await flutterLocalNotificationsPlugin.show(
        1, notificationText[0], notificationText[1], notificationDetails);
  }
  ...
  Future<void> setDefaultNotification() async {
    ...
    SharedPreferences prefs = await SharedPreferences.getInstance();
    prefs.setStringList(LS_FIELD.NOTIFICATION_TIME, ["12", "00"]);
  ...
  Future<void> unregisterScheduledNotification(
      UnRegisterCallback callback) async {

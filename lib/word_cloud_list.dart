...
class WordCloudList extends StatelessWidget {
  final Map<String, dynamic> issue;
  final double ratio;

  WordCloudList({required this.issue, required this.ratio, key}) : super(key: key);
  ...
  List<Issue> generateIssueWords(rawIssues) {
    ...
    rawIssues["topics"].forEach((issue) {
      ...
      rawIssues["totalPoint"] =
          rawIssues["totalPoint"] == 0 ? 1 : rawIssues["totalPoint"];
              ...
              child: Center(
                child: new Stack(
                  children: [
                    Center(
                        child: FittedBox(
                            fit: BoxFit.contain, child: LocationBg(location))),
                    Center(
                      child: FittedBox(
                          child:
                              WordColud(issueWords, countryCode, this.ratio, this.issue['location'], this.issue['country'])),
                    )
                  ],
                ),
              ))),
